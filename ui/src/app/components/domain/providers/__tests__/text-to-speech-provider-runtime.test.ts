...
describe('Text-to-speech provider runtime parity', () => {
  ...
  it('fills missing speaker defaults while preserving existing speaker settings', () => {
    expect(GetDefaultSpeakerConfig().map(item => item.getKey())).toEqual([
      'speaker.ambient',
    ...
    const defaults = GetDefaultSpeakerConfig([
      createMetadata('speaker.ambient', 'office'),
      createMetadata('custom.key', 'custom'),
    ]);

    expect(
      defaults.find(item => item.getKey() === 'speaker.ambient')?.getValue(),
    ).toBe('office');
    expect(
      defaults
        .find(item => item.getKey() === 'speaker.ambient_volume')
        ?.getValue(),
    ).toBe('18');
    expect(
      defaults.find(item => item.getKey() === 'custom.key')?.getValue(),
    ).toBe('custom');
  });
});
