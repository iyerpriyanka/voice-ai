...
describe('Tab', () => {
  it('selects the tab matching the active label', () => {
    ...
    expect(screen.getByText('Metadata panel').closest('[role="tabpanel"]')).not.toHaveAttribute(
      'hidden',
    );
  });
  ...
  it('changes selected panels through Carbon tabs', () => {
    ...
    expect(screen.getByText('Metrics panel').closest('[role="tabpanel"]')).not.toHaveAttribute(
      'hidden',
    );
  });
