...
describe('CarbonStatusIndicator', () => {
  it('maps known and unknown states to Carbon shape indicators', () => {
    expect(statusToShapeIndicator('SUCCESS')).toEqual({
      kind: 'stable',
      label: 'Success',
    });
    expect(statusToShapeIndicator('ERROR')).toEqual({
      kind: 'critical',
      label: 'Error',
    });
    expect(statusToShapeIndicator('MISSING')).toEqual({
      kind: 'undefined',
      label: 'Unknown',
    });

    render(<CarbonStatusIndicator state="ERROR" textSize={14} />);

    expect(screen.getByText('Error')).toHaveAttribute('data-kind', 'critical');
    expect(screen.getByText('Error')).toHaveAttribute('data-text-size', '14');
  });
